...
export async function getOwnedCardsForSets(
  ...
  try {
    ...
    return cards.map((card) => ({
      ...
      collectorNumber:
        "cardnumber" in card.card ? card.card.cardnumber.toString() : "",
  ...
  try {
    ...
    for (const change of changesWithIds) {
      if (change.newAmount < 0) {
        throw new Error(
          `Invalid amount for card ${change.setId} ${change.collectorNumber}
                     (${change.isFoil ? "foil" : "nonfoil"}): ${change.newAmount}`,
        );
      }
      const { data: accountCardId, error: accountCardError } =
      ...
      if (accountCardError) {
        console.error(
          "Error checking existing account card:",
          accountCardError,
        );
        throw accountCardError;
      }
      console.log(
        "Processing change for cardId",
        change.cardId,
        "existing accountCardId:",
        accountCardId,
        "newAmount:",
        change.newAmount,
      );
