...
export default function DeckPackages({ deckId, packages, deckCards, onChanged }: DeckPackagesProps) {
    ...
    <Box mt={4}>
      ...
      {packages.length === 0 ? (
        <Typography color="textSecondary" variant="body2">
          No packages yet. Create a package to track how well your deck covers a specific role or synergy.
        </Typography>
      ) : (
        <Box display="flex" flexWrap="wrap" gap={2}>
          {packages.map((pkg) => {
            const color = getPackageColor(pkg.id);
            const count = pkg.cardIds.length;
            ...
            return (
              ...
              >
                <CardActionArea onClick={() => openEdit(pkg)} sx={{ height: "100%", alignItems: "flex-start" }}>
                  <CardContent>
                    <Box display="flex" justifyContent="space-between" alignItems="flex-start" gap={1} mb={0.5}>
                      ...
                      <Chip
                        label={hasTarget ? `${count} / ${pkg.target}` : String(count)}
                        size="small"
                        color={hasTarget ? (fulfilled ? "success" : "warning") : "default"}
                        sx={{ flexShrink: 0 }}
                      />
                    </Box>
                    {pkg.description ? (
