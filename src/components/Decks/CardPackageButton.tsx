...
import { Avatar, Box, IconButton, List, ListItemButton, ListItemText, Popover, Tooltip, Typography } from "@mui/material";
...
export function CardPackageButton({ card, packages, onChanged }: CardPackageButtonProps) {
  ...
  return (
    <>
      <Tooltip title="Assign to packages">
        <IconButton size="small" onClick={(e: MouseEvent<HTMLButtonElement>) => setAnchorEl(e.currentTarget)}>
          ...
          {packages.length === 0 ? (
            ...
            <List dense disablePadding>
              {packages.map((pkg) => {
                ...
                return (
                  <ListItemButton key={pkg.id} onClick={() => handleToggle(pkg)} dense sx={{ borderRadius: 1, px: 0.5 }}>
                    <Avatar sx={{ width: 20, height: 20, fontSize: 9, bgcolor: getPackageColor(pkg.id), mr: 1, flexShrink: 0 }}>
                    ...
                    />
                    {inPackage && <Check fontSize="small" color="primary" sx={{ ml: 1, flexShrink: 0 }} />}
                  </ListItemButton>
                );
              })}
            </List>
