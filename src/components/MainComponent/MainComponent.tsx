...
export function MainComponent({ children }: { children: React.ReactNode }) {
  "use client";
  const { errorCode, isLoading, setAccountIdByUsername } =
    useContext(AccountContext);
  const { username } = useParams();

  useEffect(() => {
    setAccountIdByUsername(username as string)
      .then(() => {
  ...
  if (errorCode) {
    return (
      <div className={styles.page}>
        <main className={styles.main}>Something went wrong unfortunately.</main>
      </div>
    );
  }
