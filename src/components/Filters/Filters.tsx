...
export default function Filters({}): ReactNode {
  ...
  const {
    ...
    setContext,
  } = useContext(CardSelectionContextContext);
  const setsNewToOld = useMemo(
    () => applySorting(SetSorting.CHRONOLOGICAL_BACK, [...sets]),
    [sets],
  );
  ...
  function contextUpdaterForInput<T extends ChangeEvent<HTMLInputElement>>(
    setter: React.Dispatch<React.SetStateAction<string>> | undefined,
    key: string,
    targetProp: "checked" | "value",
  ) {
    return (evt: T) => {
      ...
      debounce(
        () =>
          setContext((ctx: CardSelectionContext) => ({ ...ctx, [key]: value })),
        200,
      )();
    };
  }
  ...
  function setReleasedAfterFromSet(event: SelectChangeEvent) {
    setContext((ctx: CardSelectionContext) => {
      return {
        ...ctx,
        releasedAfter:
          setsNewToOld.find((s) => s.code === event.target.value) || null,
      };
    });
  }
  ...
  function updateSortMethod(event: SelectChangeEvent) {
    setContext((ctx: CardSelectionContext) => ({
      ...ctx,
      sortingMethod: sortingMethodFromKey(event.target.value) || sortModes[0],
    }));
  }

  const setOptions = useMemo(() => {
    return sets.map(
      ({ code, name, completionRatio, numberOfCardsOwned, numberOfCards }) => {
        let label;
    ...
    <div>
        ...
        <FormGroup row={true}>
          <FormControlLabel
            ...
            control={
              <Checkbox
                checked={isFoil}
                onChange={contextUpdaterForInput(
                  undefined,
                  "isFoil",
                  "checked",
                )}
          ...
          <FormGroup row={true}>
            {colors.map(({ name }, index) => (
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    checked={activeColors.includes(name)}
                    onChange={colorToggler(name)}
                  />
                }
              />
            ))}
          ...
          <FormGroup row={true}>
            {rarities.map(({ name }, index) => (
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    checked={activeRarities.includes(name)}
                    onChange={rarityToggler(name)}
                  />
                }
              />
            ))}
      ...
      <FormControl sx={{ m: 3 }}>
        ...
        <TextField
          label="Query"
          variant="outlined"
          value={currentArtistQuery}
          onChange={contextUpdaterForInput(
            setCurrentArtistQuery,
            "artistQuery",
            "value",
          )}
        />
      ...
      {showDateFilter && (
        <>
          <FormControl sx={{ m: 3 }}>
            ...
            <Select
              value={releasedAfter?.code || ""}
              onChange={setReleasedAfterFromSet}
            >
              <MenuItem value={""} selected={releasedAfter === null}>
                (None)
              </MenuItem>
              {setsNewToOld.map(({ code, name, releaseDate }, index) => (
          ...
          <FormControl sx={{ m: 3 }}>
            ...
            <Select
              value={releasedBefore?.code || ""}
              onChange={setReleasedBeforeFromSet}
            >
              <MenuItem value={""} selected={releasedBefore === null}>
                (None)
              </MenuItem>
              {setsNewToOld.map(({ code, name, releaseDate }, index) => (
                <MenuItem key={index} value={code}>
      ...
      {sortModes.length > 1 && (
        <FormControl sx={{ m: 3 }}>
          ...
          <Select
            value={sortingMethodToKey(sortingMethod)}
            onChange={updateSortMethod}
          >
            {sortModes.map((mode, index) => (
              <MenuItem key={index} value={sortingMethodToKey(mode)}>
                {CardSortingLabels[mode]}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
