...
import RenderableMagicCardLike, {
  makeRenderable,
} from "@/interfaces/RenderableMagicCardLike";
...
export function allCardSelector(
  cards: MagicCardLike[],
  sets: CardSet[],
  ...
  if (context.isLegendary) {
    selectedCards = selectedCards.filter((card) =>
      card.card_type.includes("Legend"),
    );
  }
...
export function getTotalCardValue(
  cards: MagicCardLike[],
  context: CardSelectionContext,
): number {
  if (context.showDuplicates) {
    ...
    return cards.reduce(
      (total, card) => total + card.price_estimate * card.amount_owned,
      0,
    );
  }
}

export function getAverageTotalValue(
  cards: MagicCardLike[],
  context: CardSelectionContext,
): number {
  if (context.showDuplicates) {
    ...
    return cards.reduce(
      (total, card) => total + card.avg_price * card.amount_owned,
      0,
    );
  }
}
