...
export function setIsWithCompletion(
  set: CardSet,
): set is CardSetWithCompletion {
  const completionSet: CardSetWithCompletion = set as CardSetWithCompletion;
  return (
    typeof completionSet.completionRatio === "number" &&
    typeof completionSet.numberOfCardsOwned === "number"
  );
}

export function calculateSetCompletions(
  sets: CardSet[],
  cards: MagicCardLike[],
