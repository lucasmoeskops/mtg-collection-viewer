...
export function cardSelectionContextToHumanReadableString(
  ...
  if (context.releasedAfter) {
    parts.push(
      `released after the set ${context.releasedAfter.name} (${context.releasedAfter.releaseDate.toISOString().split("T")[0]})`,
    );
  }
  if (context.releasedBefore) {
    parts.push(
      `released before the set ${context.releasedBefore.name} (${context.releasedBefore.releaseDate.toISOString().split("T")[0]})`,
    );
  }
  if (parts.length === 0) {
    return "";
...
export function queryParametersToContext(
  query: Record<string, string>,
  sets: CardSet[],
): Partial<CardSelectionContext> {
  const newContext: Partial<CardSelectionContext> = {
    ...
    releasedBefore: query.releasedBefore
      ? (sets.find((set) => set.code === query.releasedBefore) ?? null)
      : null,
    releasedAfter: query.releasedAfter
      ? (sets.find((set) => set.code === query.releasedAfter) ?? null)
      : null,
  };

  return Object.fromEntries(
    Object.entries(newContext).filter(([key, value]) => key && value),
  );
}
