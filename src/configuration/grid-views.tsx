...
import {
  ...
  getTotalCardValue,
} from "@/procedures/card-selectors";
...
export const Browse: ViewMode = newViewMode({
  ...
  title: "Browse Mode",
  description:
    "In this view all owned cards are shown, starting with the most recent cards",
  sortModes: [
...
export const Merchant: ViewMode = newViewMode({
  ...
  title: "Merchant Mode",
  description:
    "In this view cards are ordered by their price value. Duplicate cards are also shown",
  sortModes: [
  ...
  getCardInfo: (card) => (
    <>
      <Price label="Card price" priceEstimate={card.price_estimate} />
      {card.is_foil ? <>, Foil</> : null},{" "}
      <PriceDelta
        label="Price change since july 2025"
        price={card.price_estimate}
        avgPrice={card.avg_price}
        history={{ cardId: card.id, cardName: card.name }}
      />
    </>
  ),
    ...
    <>
      ...
      <div>
        ...
        <Price
          ...
          priceEstimate={getTotalCardValue(cards, context)}
        />
        ,{" "}
        <PriceDelta
          label="Total card value change since july 2025"
          price={getTotalCardValue(cards, context)}
          avgPrice={getAverageTotalValue(cards, context)}
        />
      </div>
    </>
  ),
...
export const Collection: ViewMode = newViewMode({
  ...
  description:
    "In this view all magic cards are shown, but the cards that are not owned are fainted.",
...
export const views: ViewMode[] = [Browse, Merchant, Collection];
