...
async function _getCard(
  ...
  const scryfallCards = await fetchDataPaginated<ScryFallCard>(
    cardsSearchEndpoint,
    params,
  );
...
export function fromSupabaseCard(
  ...
  return {
    ...
    current_price_delta:
      cardData.price_estimate && card.avg_price
        ? Math.floor(cardData.price_estimate - card.avg_price)
        : 0,
    cardmarket_url: "",
  };
}
