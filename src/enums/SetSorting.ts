...
export function applySorting(sorting: SetSorting, sets: CardSet[]) {
  if (sorting === SetSorting.CHRONOLOGICAL) return sets.sort(sortChronological);
  if (sorting === SetSorting.CHRONOLOGICAL_BACK)
    return sets.sort(sortChronologicalBack);
  if (
    sorting === SetSorting.COMPLETION_BACK &&
    sets.every(setIsWithCompletion)
  ) {
...
export function sortCompletionBack(
  a: CardSetWithCompletion,
  b: CardSetWithCompletion,
): number {
